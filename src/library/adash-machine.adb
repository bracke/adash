...
package body Adash.Machine is
   ...
   --
   --  The pattern language is the one every user of a shell already has in
   --  their fingers, and no more of it than that:
   --
   --    * `*`     any run of characters, including none,
   --    * `?`     exactly one character,
   --    * `[abc]` one of those, `[a-z]` one of that range, `[!abc]` one that
   --              is none of them.
   --
   --  **Not the host's glob, and not applied to anything unasked.** Nothing
   --  here reads a directory: this compares two strings, and a script that
   --  wants the files that match one asks the filesystem for the names and
   --  asks this about each. That is the whole difference between offering
   --  matching and expanding an argument list behind a user's back, which
   --  this shell refuses and will go on refusing.
   --
   --  **Case-sensitive, on every host.** A file system that ignores case is a
   --  property of that file system, and this function is not asking one; two
   --  strings that differ are different here, as they are everywhere else in
   --  this language.
   --
   --  `/` is an ordinary character. A host writes its paths with `/` or with
   --  `\\`, so a matcher that treated one of them as a boundary would mean
   --  different things on different hosts -- and a caller comparing whole
   --  paths can say `*` and mean it.
   --
   --  Iterative, with one remembered star rather than recursion: a pattern of
   --  many stars against a long name is where the recursive version of this
   --  goes exponential, and a shell that hung on `Matches (Name, "*a*a*a*a*")`
   --  would have grown a denial of service out of a convenience.
   --
   --  @param Whole The text.
   --  @param Pattern The pattern.
   --  @return Whether the whole text matches.
   function Matched (Whole : String; Pattern : String) return Boolean;

   function Matched (Whole : String; Pattern : String) return Boolean is

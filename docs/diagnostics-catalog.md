...
| `error.handler_not_a_name` | a handler is the name of a subprogram to run, and {text} is not a name |
| `error.package_starts_a_task` | {name} starts a task when it is elaborated, so it is not carried to the next submission: carrying it would start the task again |
...
| `error.command.name_is_empty` | {name} was given nothing where it needs a name |
